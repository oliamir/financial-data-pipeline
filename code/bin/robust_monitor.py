...
QUEUE = [
    {"company": "Sofwave", "provider": "ollama", "model": "deepseek-r1:8b", "no_fallback": True, "agent": "data-analyst", "start_after": "16:00"},
    {"company": "Apollo", "provider": "ollama", "model": "llama3.1", "no_fallback": False, "agent": None}
]
...
def start_job(job_config):
    ...
    if job_config.get("agent"):
        cmd.extend(["--agent", job_config["agent"]])
            ...
            # 2. If NO process is running
                ...
                for job in QUEUE:
                    if not is_job_complete(job["company"]):
                        # Check scheduling
                        if "start_after" in job:
                            now = datetime.now()
                            # Parse HH:MM format for today
                            try:
                                target_time = datetime.strptime(job["start_after"], "%H:%M").replace(
                                    year=now.year, month=now.month, day=now.day
                                )
                                if now < target_time:
                                    print(f"Skipping {job['company']} until {job['start_after']} (Now: {now.strftime('%H:%M')})")
                                    continue
                            except ValueError:
