...
async def main():
    ...
    # Logic to handle specific year request vs years back
    target_year = None
    lookback = args.years
    
    if args.years > 1900:
        target_year = int(args.years)
        print(f"  ℹ Interpreting --years {args.years} as specific target year: {target_year}")
        current_year = datetime.now().year
        years_back_needed = current_year - target_year + 1
        lookback = max(1, years_back_needed)
    ...
    try:
        ...
        tase_reports = await tase_scraper._fetch_reports_async(years_back=lookback, company_id=company_web_id)
    ...
    # Structure: output/Company/Year/Period/
    base_download_dir = os.path.join("output", company_name.replace(" ", "_"))
    ...
    if args.years > 1900:
        ...
        current_year = datetime.now().year
        years_back_needed = current_year - target_year + 1
        # Update args.years logic for scraper call?
        # Actually, let's just pass the calculated lookback to scraper to avoid fetching too much
        # But for 'Enlight' IR scraper we can't easily control it, so we filter later.
        lookback = max(1, years_back_needed)
    else:
        lookback = args.years

    # Retroactively filter reports if needed
    if target_year:
         print(f"  Filtering reports for year {target_year}...")
         original_count = len(all_reports)
         all_reports = [r for r in all_reports if r.year == target_year]
         print(f"  Kept {len(all_reports)}/{original_count} reports matching year {target_year}")

    # OPTIMIZATION: Filter for Financial Reports only to avoid timeouts on 'Other' reports (Immediate Reports)
    # which often lack PDFs or have complex structures.
    print(f"  Filtering for Financial Reports only...")
    fin_count_before = len(all_reports)
    all_reports = [r for r in all_reports if "FINANCIAL" in r.period]
    print(f"  Kept {len(all_reports)}/{fin_count_before} FINANCIAL reports")
